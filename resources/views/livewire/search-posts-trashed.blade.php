            ...
            <div class="py-6 md:flex md:items-center md:justify-between lg:border-t lg:border-gray-200">
                ...
                <div class="mt-6 flex space-x-3 md:mt-0 md:ml-4">
                    <a href="{{ route('posts.create') }}"
                        class="button inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-gray-600 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                        Add New Post
                    </a>
                </div>
            </div>
            <div class="sm:mt-4 sm:ml-2">
                <!-- Tabs at small breakpoint and up -->
                <div class="hidden sm:block">
                    <nav class="-mb-px flex space-x-8">
                        <x-post-nav :href="route('posts.index')" :active="request()->routeIs('posts.index')">All
                        </x-post-nav>
                        <x-post-nav :href="route('posts.trash')" :active="request()->routeIs('posts.trash')">Trash
                        </x-post-nav>
                    </nav>
                </div>
            </div>
            ...
            <div class="-mx-4 overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:-mx-6 md:mx-0 md:rounded-lg">
                <table class="min-w-full divide-y divide-gray-300">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        ...
                        <tr>
                            <td class="py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{ ++$key }}</td>
                            <td
                                ...
                                <dl class="font-normal lg:hidden">
                                    ...
                                    <dd class="mt-1 truncate text-gray-500 sm:hidden">
                                        ...
                                        Published <br>
                                        {{ $item->created_at->format('d/m/Y \\a\\t\\ H:i A') }}
                                        @else
                                        Last Modified <br>
                                        {{ $item->updated_at->format('d/m/Y \\a\\t\\ H:i A') }}
                                        @endif
                                    </dd>
                                </dl>
                            </td>
                            ...
                            <td class="py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 space-y-2">
                                ...
                                <form action="{{route('posts.deletePermanent', [$item->id])}}" method="POST"
                                    class="inline-flex" onsubmit="return confirm('Delete this data permanently?')">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit"
                                        class="inline-flex justify-center rounded-md border border-transparent bg-red-600 px-3 py-2 text-sm font-medium leading-4 text-white shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
